...
exports.getConfig = function (platform, env, project_dir, argv) {
  ...
  var cfg = {
    ...
    local_tld: undefined,
    platform: platform,
    boring: true, // turn off fancy terminal stuff where possible
    // set the Hoodie instance ID, this is used to check
    // against the user roles when doing a test for Hoodie admin
    id: pkgfile.name,
    www_port: ports.getPort(this.id + '-www'),
    www_link: function () {
      return this.local_tld ? this.www_local_url : 'http://' + this.host + ':' + this.www_port;
    },
    admin_port: ports.getPort(this.id + '-admin'),
    admin_link: function () {
      return this.local_tld ? this.admin_local_url: 'http://' + this.host + ':' + this.admin_port;
    },
